...
class Shogi : Form {
  ...
  private void BuildField(){
    ...
    this.Field = new Bitmap(ChipX*9+2, ChipY*9+2);
    Graphics g = Graphics.FromImage(Field);
    g.FillRectangle(sb, 0, 0, ChipX*9, ChipY*9);
  ...
  private void BuildPiece(Piece[] pieces, bool player){
      ...
      int c = 0;
			for(int i = 0; i < 9; i++, c++){
        pieces[c] = new Piece("歩", "と金", i, 2, true, true);
				pieces[c].GetImage().RotateFlip(RotateFlipType.Rotate180FlipX);
				pieces[c].GetImage().RotateFlip(RotateFlipType.Rotate180FlipY);
				Pieces[i,2] = pieces[c];
			}
      pieces[c] = new Piece("角", "竜馬", 7, 1, true, true);
      Pieces[7,1] = pieces[c];
			pieces[c].GetImage().RotateFlip(RotateFlipType.Rotate180FlipX);
			pieces[c].GetImage().RotateFlip(RotateFlipType.Rotate180FlipY);
      c++;
      pieces[c] = new Piece("飛", "龍王", 1, 1, true, true);
      Pieces[1,1] = pieces[c];
			pieces[c].GetImage().RotateFlip(RotateFlipType.Rotate180FlipX);
			pieces[c].GetImage().RotateFlip(RotateFlipType.Rotate180FlipY);
			c++;
			for(int i = 0; i < 9; i++, c++){
        if(table2[i] == " "){
          pieces[c] = new Piece(table1[i], i, 0, true, true);
        }else{
          pieces[c] = new Piece(table1[i], table2[i], i, 0, true, true);
        }
				pieces[c].GetImage().RotateFlip(RotateFlipType.Rotate180FlipX);
				pieces[c].GetImage().RotateFlip(RotateFlipType.Rotate180FlipY);
        Pieces[i,0] = pieces[c];
